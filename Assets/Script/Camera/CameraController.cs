...
public class GameCameraController : MonoBehaviour
{
    ...
    public IEnumerator FocusBoards(
        IReadOnlyList<BoardController> boards)
    {
        if (boards == null || boards.Count == 0)
            yield break;

        Bounds combinedBounds =
            boards[0].GetWorldBounds();

        for (int i = 1; i < boards.Count; i++)
        {
            Bounds boardBounds =
                boards[i].GetWorldBounds();
